  ...
  return (
    <main className="min-h-screen bg-gray-100 p-4 sm:p-8">
      <div className="w-full max-w-7xl mx-auto">
        <header className="bg-white p-4 rounded-lg shadow-md mb-6"><div className="flex flex-col sm:flex-row justify-between sm:items-center"><div><h1 className="text-2xl sm:text-3xl font-bold text-gray-900">試合結果 vs {match?.opponent || '...'}</h1><p className="text-base text-gray-700 mt-1">{totalSets} セットの合計スタッツ</p></div><div className="flex items-center gap-3 mt-4 sm:mt-0"><div className="flex items-center p-1 bg-gray-200 rounded-lg"><button onClick={() => setViewMode('rate')} className={`px-4 py-2 text-sm font-bold rounded-md ${viewMode === 'rate' ? 'bg-white shadow' : ''}`}>率で表示</button><button onClick={() => setViewMode('count')} className={`px-4 py-2 text-sm font-bold rounded-md ${viewMode === 'count' ? 'bg-white shadow' : ''}`}>数で表示</button></div><Link href={`/matches/${matchId}`}><span className="px-4 py-2 bg-blue-600 text-white text-base font-bold rounded-md hover:bg-blue-700">記録/編集</span></Link></div></div></header>
