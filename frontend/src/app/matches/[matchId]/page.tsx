...
export default function MatchPage() {
  ...
  // --- ハンドラ関数 ---
  // (Roster, Set Start/Finish, Event Recording handlers are unchanged)
  ...
  const handleStartSet = async () => {
    ...
    try {
      ...
      await runTransaction(db, async (t) => {
        ...
        t.set(newSetRef, {
          setNumber: nextSetNumber, ourScore: 0, opponentScore: 0, status: 'ongoing',
          roster: Array.from(roster.values()), createdAt: serverTimestamp(),
        });
    ...
    try {
      ...
      await runTransaction(db, async (t) => {
        ...
        t.set(newEventRef, {
          ...
          action: selectedAction, result: result, createdAt: serverTimestamp(),
          ourScore_at_event: currentOurScore + scoreChangeOur, opponentScore_at_event: (setDoc.data().opponentScore || 0) + scoreChangeOpponent,
        });
        t.update(setRef, {
          ourScore: currentOurScore + scoreChangeOur, opponentScore: (setDoc.data().opponentScore || 0) + scoreChangeOpponent,
  ...
  const handleRecordTeamEvent = async (action: string) => {
    ...
    try {
      ...
      await runTransaction(db, async (t) => {
        ...
        const newEventRef = doc(collection(setRef, 'events'));
        t.set(newEventRef, {
          playerId: null, playerName: "チーム", position: null, action: action, result: "", createdAt: serverTimestamp(),
          ourScore_at_event: (setDoc.data().ourScore || 0) + scoreChangeOur, opponentScore_at_event: (setDoc.data().opponentScore || 0) + scoreChangeOpponent,
        });
        t.update(setRef, {
          ourScore: (setDoc.data().ourScore || 0) + scoreChangeOur, opponentScore: (setDoc.data().opponentScore || 0) + scoreChangeOpponent,
          updatedAt: serverTimestamp(),
        });
      });
  ...
  const handleOpenEditModal = (event: Event) => {
    if (!event.playerId) { alert("チームに関するプレーは、ここから編集できません。"); return; }
    const player = players.find(p => p.id === event.playerId);
    if (player) {
      setEditingEvent({ id: event.id, player, action: event.action, result: event.result });
  ...
  const getActionButtonClass = (a:string) => a.match(/スパイク|サーブ|ブロック/) ? "bg-blue-600 hover:bg-blue-700" : "bg-teal-600 hover:bg-teal-700";
      ...
      <div className="w-full max-w-5xl mx-auto">
        ...
        <header className="bg-white p-4 rounded-lg shadow-md mb-4">
          <div className="flex justify-between items-center">
            <div><h1 className="text-2xl sm:text-3xl font-bold text-gray-900">vs {match.opponent}</h1><p className="text-base text-gray-700 font-medium mt-1">{sets.map(s => `${s.ourScore}-${s.opponentScore}`).join(' / ')}</p></div>
            <div className="flex items-center gap-3"><Link href={`/matches/${matchId}/summary`}><span className="px-4 py-2 bg-gray-600 text-white text-base font-bold rounded-md hover:bg-gray-700">集計</span></Link><Link href="/dashboard"><span className="px-4 py-2 bg-blue-600 text-white text-base font-bold rounded-md hover:bg-blue-700">ダッシュボード</span></Link></div>
          </div>
        </header>

        {/* Set Selector Tabs */}
        <div className="flex items-center gap-2 mb-4 p-2 bg-white rounded-lg shadow-md overflow-x-auto">
          {sets.map(s => (<button key={s.id} onClick={() => setViewingSet(s)} className={`px-4 py-2 rounded-md font-bold text-sm whitespace-nowrap ${viewingSet?.id === s.id ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800 hover:bg-gray-300'}`}>Set {s.setNumber} {s.status === 'ongoing' ? ' (記録中)' : ''}</button>))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="bg-white p-4 rounded-lg shadow-md flex justify-around items-center">
                <div className="text-center"><p className="text-xl font-bold text-gray-800">自チーム</p><p className="text-6xl font-bold text-blue-600 tracking-tighter">{viewingSet.ourScore}</p></div>
                <div className="text-center">
                  ...
                  <div className="flex flex-wrap justify-center gap-2 mt-2">
                    ...
                    {viewingSet.status === 'finished' && <button onClick={() => handleOpenRosterModal(viewingSet)} className="px-3 py-2 bg-purple-600 text-white text-sm font-bold rounded-md hover:bg-purple-700">ロスター編集</button>}
                  </div>
                </div>
                <div className="text-center"><p className="text-xl font-bold text-gray-800">相手チーム</p><p className="text-6xl font-bold text-red-600 tracking-tighter">{viewingSet.opponentScore}</p></div>
              </div>

              {viewingSet.status === 'ongoing' ? (<>
                <div className="grid grid-cols-3 sm:grid-cols-3 gap-4">{viewingSet.roster.filter(p => p.position !== 'SUB').map(player => (<div key={player.playerId} onClick={() => handlePlayerTileClick(player)} className="bg-white p-4 rounded-lg shadow-md text-center cursor-pointer hover:bg-blue-50"><p className="font-bold text-xl text-gray-900">{player.displayName}</p><p className="text-base text-blue-800 font-semibold">{player.position}</p></div>))}</div>
                <div className="grid grid-cols-2 gap-4"><button onClick={() => handleRecordTeamEvent(TEAM_ACTIONS.OPPONENT_ERROR)} className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 rounded-lg shadow-md text-lg">相手のミス</button><button onClick={() => handleRecordTeamEvent(TEAM_ACTIONS.OUR_ERROR)} className="w-full bg-red-500 hover:bg-red-600 text-white font-bold py-4 rounded-lg shadow-md text-lg">こちらのミス</button></div>
              </>) : (<div className="bg-white p-4 rounded-lg shadow-md"><h3 className="text-xl font-bold text-center text-gray-800 mb-2">このセットは終了しました</h3><p className="text-center text-gray-600">プレーを編集するには「全履歴」から操作してください。</p></div>)}
            </div>
            {/* Event Log */}
            <div className="bg-white p-4 rounded-lg shadow-md"><h2 className="text-xl font-semibold mb-4 text-gray-800">プレー履歴 (Set {viewingSet.setNumber})</h2><ul className="space-y-3">{events.map((event) => (<li key={event.id} onClick={() => handleOpenEditModal(event)} className="p-3 rounded-md hover:bg-gray-100 cursor-pointer border-b"><p className="font-semibold text-base text-gray-800">{event.playerName}: <span className="font-medium text-gray-700">{event.action} - {event.result}</span></p><p className="text-sm text-gray-600 mt-1">{event.createdAt?.toDate().toLocaleTimeString()}</p></li>))}</ul></div>
          </div>
          ...
          <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md">
            ...
            <div className="space-y-5">
              <div>
                ...
                <select id="player-out" value={playerOutId} onChange={(e) => setPlayerOutId(e.target.value)} className="w-full mt-1 border border-gray-300 p-3 rounded-md text-gray-900 text-base">
              ...
              <div>
                ...
                <select id="player-in" value={playerInId} onChange={(e) => setPlayerInId(e.target.value)} className="w-full mt-1 border border-gray-300 p-3 rounded-md text-gray-900 text-base">
                  <option value="" disabled>選択してください</option>
                  {subPlayers.map(p => (
